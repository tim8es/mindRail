...
describe('D1RuntimePersistence ordering, isolation, and history invariants', () => {
  ...
  it('keeps checkpoints and audit history ordered and immutable', async () => {
    ...
    expect((await persistence.listTaskCheckpoints(seeded.workspace.id, seeded.task.id)).map((item) => item.id)).toEqual([
      'checkpoint-1',
      'checkpoint-2',
    ]);
    expect((await persistence.listAuditEvents(seeded.workspace.id, 10)).map((item) => item.id)).toEqual([
      'audit-1',
      'audit-2',
    ]);
  ...
  it('supports pending-human permission reads and immutable decision-head advancement without evaluating policy', async () => {
    ...
    const initialDecision = policyDecision(seeded.workspace.id, request.id, 'permission-decision-a');
  ...
  it('exposes authoritative recovery reads for active leases past expiry and sessions past liveness cutoff', async () => {
    ...
    expect(await persistence.listExpiredActiveLeases('ws-a', T1, 10)).toEqual([
      claim.value.lease,
    ]);
    expect(await persistence.listActiveSessionsLastSeenBefore('ws-a', T1, 10)).toEqual([
      session(),
    ]);
    database.close();
  });
