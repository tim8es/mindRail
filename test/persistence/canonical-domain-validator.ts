...
import type {
  PersistenceDomainTarget,
  PersistenceDomainValidator,
...
const schemaRefs: Record<PersistenceDomainTarget, string> = {
  ...
  AuditEvent: 'urn:mindrail:schema:domain:v1:audit-event',
};
...
const validators = new Map<PersistenceDomainTarget, ValidateFunction>();
for (const [target, ref] of Object.entries(schemaRefs) as [PersistenceDomainTarget, string][]) {
  validators.set(target, ajv.getSchema(ref) ?? ajv.compile({ $ref: ref }));
}

export const persistenceCanonicalValidator: PersistenceDomainValidator = (target, value) => {
