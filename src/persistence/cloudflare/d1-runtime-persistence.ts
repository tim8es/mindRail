...
export class D1RuntimePersistence implements DurableRuntimePersistence {
  ...
  async claimTask(
    ...
    return this.coordinator.runSerialized(input.workspaceId, async () => {
      ...
      if (timestampMs(lease.expiresAt, 'Lease.expiresAt') <= nowMs) {
        throw new PersistenceError('INVALID_RECORD', 'A newly granted Lease must expire in the future.');
      }
  ...
  async updateTask(input: { task: Task; expectedRevision: number }): Promise<Task> {
    ...
    return this.coordinator.runSerialized(task.workspaceId, async () => {
      ...
      if (task.goalId !== current.goalId) {
        throw new PersistenceError('INVALID_RECORD', `Task ${task.id} cannot change Goal ownership.`);
      }
  ...
  async completeTask(
    ...
    return this.coordinator.runSerialized(input.workspaceId, async () => {
      ...
      if (!currentLease) {
        throw new PersistenceError('STALE_AUTHORITY', `Lease ${input.lease.id} is not authoritative.`);
      }
  ...
  private async assertCheckpointAuthority(checkpoint: Checkpoint, nowMs: number): Promise<void> {
    ...
    if (!effective || effective.id !== lease.id || effective.fencingToken !== checkpoint.fencingToken) {
  ...
  private assertCompletionAuthority(
    ...
    if (
      ...
      throw new PersistenceError('STALE_AUTHORITY', 'Task completion authority is stale or invalid.');
    }
  }
  ...
  private assertReceipt(workspaceId: string, receipt: CommandReceiptInput | undefined): void {
    ...
    if (receipt.workspaceId !== workspaceId) {
      throw new PersistenceError('INVALID_RECORD', 'Command receipt Workspace does not match mutation.');
    }
    if (!receipt.commandId || !receipt.command || !receipt.semanticFingerprint) {
      throw new PersistenceError('INVALID_RECORD', 'Command receipt identity fields must be non-empty.');
    }
    ...
    serializeJson(receipt.responseSnapshot, 'CommandReceipt.responseSnapshot', MAX_RECEIPT_SNAPSHOT_BYTES);
  }
  ...
  private async requireWorkspace(workspaceId: string): Promise<Workspace> {
    ...
    if (!workspace) throw new PersistenceError('NOT_FOUND', `Workspace ${workspaceId} was not found.`);
    if (workspace.status !== 'active') {
      throw new PersistenceError('INVALID_STATE_TRANSITION', `Workspace ${workspaceId} is archived.`);
    }
    return workspace;
  }
  ...
  private async getFencingCounter(workspaceId: string, taskId: string): Promise<number | undefined> {
  ...
  private async first<T>(sql: string, ...values: unknown[]): Promise<T | undefined> {
    const row = await this.database.prepare(sql).bind(...values).first<T>();
