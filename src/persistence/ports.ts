...
export interface DurableRuntimePersistence {
  ...
  getCommandReceipt(workspaceId: string, commandId: string): Promise<StoredCommandReceipt | undefined>;
  ...
  listPendingHumanPermissions(workspaceId: string, limit: number): Promise<PendingHumanPermission[]>;
  listExpiredActiveLeases(workspaceId: string, now: string, limit: number): Promise<Lease[]>;
  listActiveSessionsLastSeenBefore(workspaceId: string, cutoff: string, limit: number): Promise<Session[]>;
}
