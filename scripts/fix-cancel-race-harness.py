...
text = path.read_text()
start = text.index("  it('rejects a stale CancelGoal snapshot when durable Task membership changes before its batch'")
end = text.index('\n  });\n});', start) + len('\n  });')
replacement = r'''  it('rejects a stale CancelGoal snapshot when an independent Task commit changes membership before its batch', async () => {
