...
create_task = r'''  async createTask(input: {
    task: Task;
...
replace_region('  async createTask(input: {', '  async claimTask(', create_task)

retry_task = r'''  async retryTask(input: {
    task: Task;
...
replace_region('  async retryTask(input: {', '  async cancelTask(', retry_task)

cancel_task = r'''  async cancelTask(
...
replace_region('  async cancelTask(', '  async cancelGoal(', cancel_task)

cancel_goal = r'''  async cancelGoal(
...
replace_region('  async cancelGoal(', '  private async commitTaskOutcome(', cancel_goal)

helper_marker = '''  private async readRecord<T>(sql: string, ...values: unknown[]): Promise<T | undefined> {'''
helpers = r'''  private mutationChangesGuardStatement(workspaceId: string): D1PreparedStatementLike {
...
if text.count(helper_marker) != 1:
    raise RuntimeError('readRecord marker mismatch')
text = text.replace(helper_marker, helpers + helper_marker, 1)

old_batch = r'''  private async batch(
...
new_batch = r'''  private async batch(
    statements: D1PreparedStatementLike[],
    context: string,
  ): Promise<D1ResultLike[]> {
    try {
...
if text.count(old_batch) != 1:
    raise RuntimeError('batch method marker mismatch')
