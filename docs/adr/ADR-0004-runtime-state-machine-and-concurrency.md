...
ADR-0003 separates durable Task state from Session and Lease ownership, requires optimistic revisions for mutable resources, and requires monotonically increasing fencing tokens for successive Task leases. This ADR defines the deterministic runtime semantics that persistence and protocol adapters must preserve.
