...
`commandId` is the protocol idempotency key.
...
Requires active Session/Agent, capability match, current observed Task revision when required, and Task state:

- `ready`; or
- `running` without an effective Lease.
...
Requires authorized actor, current Task revision, Task exactly `failed`, active Goal, and no effective Lease. ADR-0004 guarantees dependencies are already satisfied, so v0.1 transitions only:

```text
failed -> ready
```

No automatic retry/backoff is introduced.
